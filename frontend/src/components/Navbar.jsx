  ...
  return (
    <header
      ...
      className={`fixed top-0 inset-x-0 z-50 transition-colors duration-500 ${
        scrolled ? "bg-[#070707]/70 backdrop-blur-xl border-b border-white/10" : "bg-transparent border-b border-transparent"
      }`}
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 h-20 flex items-center justify-between">
        <Link to="/" data-testid="nav-logo" className="text-2xl text-white hover:opacity-80 transition-opacity">
          <Wordmark />
        </Link>

        <nav className="hidden md:flex items-center gap-9 font-mono-tech text-xs uppercase tracking-widest text-zinc-400">
          {navItems.map((item) => (
            <button
              key={item.id}
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <Link
            ...
            className="hidden md:inline-flex items-center gap-2 bg-white text-black px-5 py-2.5 font-mono-tech text-xs uppercase tracking-widest hover:bg-[#0044ff] hover:text-white transition-colors duration-300"
          >
            {t.nav.quote}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="md:hidden overflow-hidden bg-[#070707] border-b border-white/10"
          >
